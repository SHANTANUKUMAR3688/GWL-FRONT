...
const AddManger = () => {
    ...
    const handlesubmit = async (e) => {
        e.preventDefault();
        try {
            await API.post("/create-lowermanager", data, {
                headers: {
    ...
    const getLastLowerManId = async () => {
        const response = await API.get("/getLastLowerMan");
        const nextNumber = parseInt(response.data.lastLowerManId.replace("LM", "")) + 1;
        ...
        setdata({
            ...
            lastname: "",
            phone: "",
            managerid: "LM" + String(nextNumber).padStart(2, '0')
        });
    }
